...
fn main() {
    ...
    println!("🚀 ~ file: vrf.rs ~ line 17 ~ fnmain ~ secret_key {:?}", secret_key);
    let public_key = vrf.derive_public_key(&secret_key).unwrap();
    println!("🚀 ~ file: vrf.rs ~ line 20 ~ fnmain ~ public_key {:?}", public_key);
    let message: &[u8] = b"sample";
    println!("🚀 ~ file: vrf.rs ~ line 21 ~ fnmain ~ message {:?}", message);

    // Generation of VRF proof and hash output
    let pi = vrf.prove(&secret_key, &message).unwrap();
    let hash = vrf.proof_to_hash(&pi).unwrap();
    println!("🚀 ~ file: vrf.rs ~ line 26 ~ fnmain ~ hash {:?}", hash);
    println!("Generated VRF proof: {}", hex::encode(&pi));

    // Invoking Verify Method: VRF proof verification (returns VRF hash output)
    let beta = vrf.verify(&public_key, &pi, &message);
    println!("🚀 ~ file: vrf.rs ~ line 30 ~ fnmain ~ beta {:?}", beta);

    match beta {
        Ok(beta) => {
            println!("VRF proof is valid!");
            println!("Hash-beta output: {}", hex::encode(&beta));
            assert_eq!(hash, beta);
        }
        Err(e) => {
