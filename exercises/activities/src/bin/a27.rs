...
enum ProgramError {
    #[error("MenuError")]
    ProgramMenuError(#[from] MenuError), // enum inside enum NewVariantName(OldVariantName) ==> ProgramMenuError(MenuError) ===> additional #from required
    // #from required because it converts MenuError to ProgramError
    // error that'll come if #from annotation isn't used : expected `ProgramError` :  the trait `From<MenuError>` is not implemented for `ProgramError`
    #[error("MathError")]
    ProgramMathError(#[from] MathError),
}
